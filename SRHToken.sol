...
contract MySRHToken {
    ...
    function transfer(address beneficiary, uint256 amount) public returns (bool) {
        require(beneficiary != address(0), "Beneficiary address can not be zero. Please enter valid address!");
    ...
    function approve(address spender, uint256 amount) public returns (bool success) {
        require(spender != address(0), "Spender address can not be zero, Please enter valid address!");
    ...
    function transferFrom(address sender, address beneficiary, uint256 amount)  public returns (bool) {
        require(sender != address(0), "Sender address can not be zero.");
        require(beneficiary != address(0), "Beneficiary address can not be zero. Please enter valid address!");
...
contract SRHToken is MySRHToken, Administrable {
    mapping (address => bool) private _frozenAccounts;
    mapping (address => uint) private _pendingWithdrawals;

    uint256 private _sellPrice = 1; // Here just mention that how many ehter would be per token.
   uint256 private _buyPrice = 1;  // Here just mention that how many ehter would be per token.
